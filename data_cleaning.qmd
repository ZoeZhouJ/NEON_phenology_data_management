...
status_core <- status %>%
  select(
  individualID, uid, date, eventID, phenophaseName, phenophaseStatus,   phenophaseIntensityDefinition, phenophaseIntensity,remarks    
  ) %>% 
...
## Check tables for schema building
```{r}
glimpse(ind_cleaned)
```
```{r}
...
```{r}
# Check output
plant <- read.csv("./phenology_csv/phe_perindividual.csv")
census <- read.csv("./phenology_csv/phe_perindividualperyear.csv")
phenophase_obs <- read.csv("./phenology_csv/phe_statusintensity.csv")
